...
class VideoDataset(Dataset):
    def __init__(self, data_dir, is_test = False, img_size=(512, 512), rolling_window=5):
        ...
        for key in self.keys:
            self.files_per_key[key] = [os.path.join(self.data_dir, 'downsampled', key, file) for file in self.files_per_key[key]]
    ...
    def prepare_data(self):
        ...
        for key in tqdm(self.total_keys):
            ...
            images = [img_dir for img_dir in sorted(os.listdir(os.path.join(self.data_dir, key))) if 'downsampled' not in img_dir]
            for file in images:
                with Image.open(os.path.join(self.data_dir, key, file)) as img:
                    img = torch.nn.functional.interpolate(
                        TF.ToTensor()(img).unsqueeze(0), scale_factor= 1/4, mode="bicubic"
                    )
    ...
    def __len__(self):
        ...
        return sum([self.num_files_per_key[key] - self.rolling_window + 1 for key in self.keys])
    ...
    def __getitem__(self, index):
        ...
        file_names = self.files_per_key[key][file_idx - (self.rolling_window - 1)//2: file_idx + (self.rolling_window - 1)//2 + 1]
        # gt_image
        gt_image = self.read_image(os.path.join(self.data_dir, key, f'frame_{file_idx:04d}.png'))
        # read the images
    ...
    def get_key_and_file_idx(self, index):
        ...
        for key in self.keys:
            if index < self.num_files_per_key[key] - self.rolling_window + 1:
                return key, index + (self.rolling_window - 1)//2 # add the offset to get the index of the middle frame
            else:
...
def generate_segment_indices(videopath1,videopath2,num_input_frames=10,filename_tmpl='{:08d}.png'):
    ...
    segment1=[self.read_image(os.path.join(videopath1,filename_tmpl.format(i))) / 255. for i in range(start_frame_idx,end_frame_idx)]
    segment2=[self.read_image(os.path.join(videopath2,filename_tmpl.format(i))) / 255. for i in range(start_frame_idx,end_frame_idx)]
